...
# function
def travel(cur, visited, N, MAX_VISITED):	
	# base case
	if opt[cur][visited] is not None:
		return opt[cur][visited]
	# 모든 도시를 방문했음
	elif visited == MAX_VISITED and travel_cost[cur][0] > 0: 
	...
	for next in range(N):
		...
		# 방문하지 않았는지 확인
		if ((MAX_VISITED - 2 ** next) | visited) == MAX_VISITED:
			continue

		val = travel(next, visited | 2 ** next, N, MAX_VISITED)
